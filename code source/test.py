...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def Executer(self, fichier):
        ...
        if self.selected_text == "BETHUNE":
            # Logique spécifique pour corriger les totaux de Bethune - même correction que les autres agences
            print("Correction spécifique des totaux pour Bethune")
            logging.info("Correction spécifique des totaux pour Bethune")
            ...
            if not ligne_BRUT_a_payer.empty and not resultat_fusion_2.empty:
                ...
                if not brut_total_line.empty:
                    ...
                    # Mettre à jour aussi dans resultat_fusion_3
                    resultat_fusion_3.loc[resultat_fusion_3['Titre'].str.contains('BRUT à payer', case=False, na=False), 'A Remplir'] = brut_total_value
                    print(f"BRUT à payer corrigé pour Bethune: {brut_total_value}")
                    logging.info(f"BRUT à payer corrigé pour Bethune: {brut_total_value}")
        ...
        if self.selected_text == "CREIL":
            # Logique spécifique pour corriger les totaux de Creil - même correction que les autres agences
            print("Correction spécifique des totaux pour Creil")
            logging.info("Correction spécifique des totaux pour Creil")
            ...
            if not ligne_BRUT_a_payer.empty and not resultat_fusion_2.empty:
                ...
                if not brut_total_line.empty:
                    ...
                    # Mettre à jour aussi dans resultat_fusion_3
                    resultat_fusion_3.loc[resultat_fusion_3['Titre'].str.contains('BRUT à payer', case=False, na=False), 'A Remplir'] = brut_total_value
                    print(f"BRUT à payer corrigé pour Creil: {brut_total_value}")
                    logging.info(f"BRUT à payer corrigé pour Creil: {brut_total_value}")
            ...
            if not ligne_Fiscal.empty and valeur_fiscal_originale is not None:
                ...
                # Mettre à jour aussi dans resultat_fusion_3
                resultat_fusion_3.loc[resultat_fusion_3['Titre'].str.contains('Fiscal', case=False, na=False), 'A Remplir'] = valeur_fiscal_originale
                print(f"Total Fiscal corrigé pour Creil avec la valeur originale: {valeur_fiscal_originale}")
                logging.info(f"Total Fiscal corrigé pour Creil avec la valeur originale: {valeur_fiscal_originale}")
        ...
        if excel_file_3 is not None:
            ...
            if ligne_Jrubriques:
                ...
                for index, row in resultat_fusion_3.iterrows():
                    ...
                    # Logique spéciale pour Bethune - recherche plus précise
                    elif self.selected_text == "BETHUNE":
                        if 'BRUT à payer' in str(row['Titre']):
                            # Chercher spécifiquement la ligne contenant "BRUT à payer"
                            for row_idx in range(1, ws.max_row + 1):
                                cell_value = ws.cell(row=row_idx, column=2).value  # Chercher dans la colonne 2 (Titre)
                                if cell_value and 'BRUT à payer' in str(cell_value):
                                    ws.cell(row=row_idx, column=3, value=valeur_a_ecrire)
                                    print(f"✅ BETHUNE - Écrit '{valeur_a_ecrire}' à la ligne {row_idx} pour 'BRUT à payer'")
                                    ligne_trouvee = True
                                    break
                        elif 'Fiscal' in str(row['Titre']) and 'BRUT' not in str(row['Titre']):
                            # Chercher spécifiquement la ligne contenant "Fiscal" mais pas "BRUT"
                    ...
                    # Logique spéciale pour Creil - recherche plus précise
                    elif self.selected_text == "CREIL":
                        if 'BRUT à payer' in str(row['Titre']):
                            # Chercher spécifiquement la ligne contenant "BRUT à payer"
                            for row_idx in range(1, ws.max_row + 1):
                                cell_value = ws.cell(row=row_idx, column=2).value  # Chercher dans la colonne 2 (Titre)
                                if cell_value and 'BRUT à payer' in str(cell_value):
                                    ws.cell(row=row_idx, column=3, value=valeur_a_ecrire)
                                    print(f"✅ CREIL - Écrit '{valeur_a_ecrire}' à la ligne {row_idx} pour 'BRUT à payer'")
                                    ligne_trouvee = True
                                    break
                        elif 'Fiscal' in str(row['Titre']) and 'BRUT' not in str(row['Titre']):
                            # Chercher spécifiquement la ligne contenant "Fiscal" mais pas "BRUT"
                            for row_idx in range(1, ws.max_row + 1):
                                ...
                                if cell_value and 'Fiscal' in str(cell_value) and 'BRUT' not in str(cell_value):
                                    ws.cell(row=row_idx, column=3, value=valeur_a_ecrire)
                                    print(f"✅ CREIL - Écrit '{valeur_a_ecrire}' à la ligne {row_idx} pour 'Fiscal'")
                                    ligne_trouvee = True
                                    break
